...
const WisataPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Mencegah ID Collision dengan Prefix
  ...
  const filteredWisata = useMemo(() => {
    return wisataList.filter((w) => {
      ...
      // Filter fleksibel untuk tag lama (db) dan tag baru (UI)
      const matchTag = !activeTag || tags.some(t => 
        t === activeTag || 
    ...
    <section className="bg-pageRadial min-h-screen pb-20 px-4">
      <div className="max-w-6xl mx-auto pt-10">
        ...
        <div className="mt-6 bg-white rounded-full border px-6 py-3 shadow-sm flex items-center gap-3">
          <input className="w-full outline-none" placeholder="Cari wisata..." value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>

        <div className="mt-4 flex flex-wrap gap-2">
          {["Wisata Alam", "Wisata Pendidikan"].map((cat) => (
            <button key={cat} onClick={() => setActiveKategori(activeKategori === cat ? null : cat)}
          ...
          {filteredWisata.map((w) => (
            <Link key={w.uniqueId} to={`/wisata/${w.uniqueId}`}>
              <div className="bg-white rounded-[32px] shadow-lg overflow-hidden h-full flex flex-col hover:-translate-y-1 transition">
                <img src={w.link_foto !== "-" ? w.link_foto : "https://placehold.co/600x400"} className="h-52 w-full object-cover" />
                <div className="p-6 flex-1 flex flex-col">
                  ...
                  <div className="mt-auto pt-4 flex gap-2">
                    {Array.isArray(w.tags) && w.tags.map((t: string) => tagConfig[t] && (
                      <div key={t} className="p-2 bg-slate-50 rounded-full text-slate-500" title={tagConfig[t].label}>{tagConfig[t].icon}</div>
                    ))}
                  </div>
                </div>
              </div>
